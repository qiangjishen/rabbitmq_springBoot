...
import apache.dubbo.springboot.demo.provider.config.LazyRabbitConfig;
...
public class LazyController {
    ...
    public void sendMsg(@PathVariable("msg") String msg) {
        ...
        c1.setRealName("乔帮主");
        c1.setAddress("西夏");
        c1.setEmail("mask@twitter.com");

        list.add(c);
        list.add(c1);

        String mm = JSONObject.toJSONString(list);

        sendMsg(LazyRabbitConfig.LAZY_TOPIC_EXCHANGE,"cnnic.topic.test",mm, null);
    }
    ...
    private final RabbitTemplate.ConfirmCallback confirmCallback = new RabbitTemplate.ConfirmCallback() {
        ...
        public void confirm(CorrelationData correlationData, boolean ack, String cause) {
            if(!ack){
                //可以进行日志记录、异常处理、补偿处理等
                log.info("异常ack-"+ack+",id-"+correlationData.getId()+",cause:"+cause);
            }else {
                //更新数据库，可靠性投递机制
                log.info("正常ack-"+ack+",id-"+correlationData.getId());
                try{
    ...
    private final RabbitTemplate.ReturnCallback returnCallback = new RabbitTemplate.ReturnCallback() {
        ...
        public void returnedMessage(Message message, int replyCode, String replyText, String exchange, String routingKey) {
            log.info("spring_returned_message_correlation:"+message.getMessageProperties().getHeaders().get(PublisherCallbackChannel.RETURNED_MESSAGE_CORRELATION_KEY)
