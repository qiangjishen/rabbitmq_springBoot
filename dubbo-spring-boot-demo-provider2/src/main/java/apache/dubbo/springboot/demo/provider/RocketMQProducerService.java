...
public class RocketMQProducerService {
    ...
    // 建议正常规模项目统一用一个TOPIC
    private static final String topic = "CNNIC_ROCKETMQ_TOPIC1";

    private static final String topic2 = "CNNIC_ROCKETMQ_TOPIC2";
     ...
     */
    public void send(CustomerDto user) {
...
//        rocketMQTemplate.send(topic + ":tag1", MessageBuilder.withPayload(user).build()); // 等价于上面一行
    ...
    public void sendAsyncMsg(String msgBody) {
        ...
        rocketMQTemplate.asyncSend(topic, MessageBuilder.withPayload(msgBody).build(), new SendCallback() {
            ...
            public void onSuccess(SendResult sendResult) {
                ...
                System.out.println("往RocketMQ发送成功，执行其他逻辑");
            }
            @Override
            public void onException(Throwable throwable) {
                // 处理消息发送异常逻辑
                System.out.println("往RocketMQ发送失败。。。。。。");
    ...
    public void sendMessageInTransaction(String msg) {
        Message<String> message = MessageBuilder.withPayload(msg).setHeader(RocketMQHeaders.TRANSACTION_ID,"tx-cnnic-0001").build();

        rocketMQTemplate.sendMessageInTransaction(topic+":tag1",message,null);
    }

    public void sendMessageInTransaction2(String msg) {
        Message<String> message = MessageBuilder.withPayload(msg).setHeader(RocketMQHeaders.TRANSACTION_ID,"tx-cnnic-0002").build();
        DefaultMQProducer p = new DefaultMQProducer("");


        rocketMQTemplate.sendMessageInTransaction(topic2,message,null);
    }
}
